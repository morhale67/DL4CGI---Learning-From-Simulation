...
from DataFunctions import build_dataset, get_data
import torch
...
def train(params, log_path, folder_path, wb_flag):
    ...
    train_loader, test_loader = get_data(params['pic_width'], params['n_masks'], params['batch_size'], params['n_samples'])
    ...
    for epoch in range(params['epochs']):
        ...
        train_loss_epoch, train_psnr_epoch, train_ssim_epoch = train_epoch(epoch, network, train_loader, optimizer,
                                                         params['batch_size'], params['img_dim'], params['n_masks'], device, log_path, folder_path,
                                                         wb_flag, save_img=params['save_img'])
        print_training_messages(epoch, train_loss_epoch, lr, start_epoch, log_path)
        test_loss_epoch, test_psnr_epoch, test_ssim_epoch = test_net(epoch, network, test_loader, params['n_masks'], device, log_path, folder_path,
                                                    params['batch_size'], params['img_dim'], wb_flag, save_img=params['save_img'])
